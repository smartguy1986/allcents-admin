...
class Payments extends REST_Controller
{
    ...
    public function requestpayment_post()
    {
        ...
        if (strtolower($paymentmode) == 'cash') {
            ...
            if (strtolower($resparray[0]['status']) == 'ok') {
                ...
                storemylog("Payment Gateway URL Generation for donation", "api/v2/payments/requestpayments", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($val));
            } else {
                $main_arr['info']['response']['status'] = '0';
                $main_arr['info']['response']['data'] = '';
                $main_arr['info']['response']['message'] = 'Payment was not successful. Please try again.';

                storemylog("Payment Gateway URL Generation for donation", "api/v2/payments/requestpayments", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Payment was not successful. Please try again.');
            }
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['data'] = '';
            ...
            storemylog("Payment Gateway URL Generation for donation", "api/v2/payments/requestpayments", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Payment mode not supported');
        }
    ...
    public function makecashpayment_post()
    {
        ...
        storemylog("Cash Payment by Treasurer on behalf of User", "api/v2/payments/makecashpayment", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], 'Thank you for registering cash donation on behalf of ' . $udata->usertitle . ' ' . $udata->userfname . ' ' . $udata->userlname);
