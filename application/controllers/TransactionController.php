...
class TransactionController extends CI_Controller
{
    ...
    {
        $data['transactions'] = $this->AdminModel->get_transaction_info();
        $this->load->view('admin/transaction/transactionlist', $data);
    }

    public function add()
    {
        $data['transactions'] = $this->AdminModel->get_donation_info();
        $this->load->view('admin/transaction/makedonation', $data);
    }

    public function register()
    {
        $transid = $this->input->post('transid');
        $userid = $this->input->post('donate_by');
        ...
        if ($agreecheck) {
            ...
            $this->AdminModel->update_policypurchase($conditions2, $purchase_id);
            $id = $this->AdminModel->update_transaction($conditions, $transid);
            if ($id) {
                $this->session->set_flashdata('success', 'Transaction registered successfully!');
                $udata = get_user_number($userid);
                sndsmstouserlive($udata->country_code . $udata->userphone, 'Congratulations! Your Policy premium of amount $ ' . $transaction_amount . ' has been successfully submitted to church account.', $userid);
                $notArray = array(
                    "not_title" => "Payment Accepted",
                    "not_msg" => 'Congratulations! Your Policy premium of amount $ ' . $transaction_amount . ' has been successfully submitted to church account.',
                    "not_uid" => $userid,
                    "is_read" => "0"
                );
                ...
                $this->session->set_flashdata('error', 'Transaction not registered. Please try again!');
                redirect('transactions');
            }
        }
    }
