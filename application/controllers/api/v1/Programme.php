...
class Programme extends REST_Controller
{
    ...
    public function getcategories_post()
    {
        ...
        if (!empty($category)) {
            ...
            storemylog("Programme Category Details", "api/v1/programme/getcategories", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($category));
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = '';
            $main_arr['info']['response']['data'] = 'No categories Found.';

            storemylog("Programme Category Details", "api/v1/programme/getcategories", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No categories Found.');
        }
    ...
    public function getprogramme_post()
    {
        ...
        if (count($topics) > 0) {
            ...
            storemylog("Programme Lists by Category", "api/v1/programme/getprogramme", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($topicArray));
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = '';
            $main_arr['info']['response']['data'] = 'No Programmes Found.';

            storemylog("Programme Lists by Category", "api/v1/programme/getprogramme", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No Programmes Found.');
        }
        $this->response($main_arr, REST_Controller::HTTP_OK);
    }
    ...
    public function getprogrammedetails_post()
    {
        ...
        if (count($topics) > 0) {
            ...
            storemylog("Programme Details by Id", "api/v1/programme/getprogrammedetails", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($event));
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = '';
            $main_arr['info']['response']['data'] = 'No Programmes Found.';

            storemylog("Programme Details by Id", "api/v1/programme/getprogrammedetails", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No Programmes Found.');
        }
        $this->response($main_arr, REST_Controller::HTTP_OK);
    }
