...
class Notices extends REST_Controller
{
    ...
    public function getcategories_post()
    {
        ...
        if (!empty($category)) {
            ...
            storemylog("Notices Category Details", "api/v1/notices/getcategories", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($category));
        } else {
            ...
            storemylog("Notices Category Details", "api/v1/notices/getcategories", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No categories Found.');
        }
    ...
    public function getnotices_post()
    {
        ...
        if (count($topics) > 0) {
            ...
            storemylog("Notices Lists by Category", "api/v1/notices/getnotices", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], json_encode($topics));
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = '';
            $main_arr['info']['response']['data'] = 'No Notices Found.';

            storemylog("Notices Lists by Category", "api/v1/notices/getnotices", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No Notices Found.');
        }
        $this->response($main_arr, REST_Controller::HTTP_OK);
    }
    ...
    public function getnoticedetails_post()
    {
        ...
        if (count($topics) > 0) {
            ...
            storemylog("Notice Details by Id", "api/v1/notices/getnoticedetails", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($event));
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = '';
            $main_arr['info']['response']['data'] = 'No Programmes Found.';

            storemylog("Notice Details by Id", "api/v1/notices/getnoticedetails", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No Programmes Found.');
        }
        $this->response($main_arr, REST_Controller::HTTP_OK);
    }
    ...
    public function notifications_post()
    {
        ...
        if (empty($uid)) {
            ...
            storemylog("Notifications List", "api/v1/notices/notifications", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'User id is empty');
        } else {
            $this->db->select("*");
            ...
            if (count($topics) > 0) {
                ...
                storemylog("Notifications List", "api/v1/notices/notifications", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($new_array));
            } else {
                $main_arr['info']['response']['status'] = '0';
                ...
                storemylog("Notifications List", "api/v1/notices/notifications", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], json_encode($new_array));
            }
        }
