...
class Funeral extends REST_Controller
{
    ...
    public function funeralpage_post()
    {
        ...
        if (empty($userdata)) {
            ...
            storemylog("'Content Details of user\'s Funeral page'", "api/v1/funeral/funeralpage", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($data));
        }
    ...
    public function policylist_post()
    {
        ...
        if (empty($policies)) {
            ...
            storemylog("List of Funeral Policies", "api/v1/funeral/policylist",  "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No Policy Found');
        } else {
            ...
            storemylog("List of Funeral Policies", "api/v1/funeral/policylist",  "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($policies));
        }
    ...
    public function policylist_treasurer_post()
    {
        ...
        if (empty($policies)) {
            ...
            storemylog("List of Funeral Policies for Treasure", "api/v1/funeral/policylist_treasurer", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'No Policy Found');
        } else {
            foreach ($policies as $policy) {
                $this->db->select('*');
                $this->db->where('policy_id', $policy->id);
            ...
            storemylog("List of Funeral Policies for Treasure", "api/v1/funeral/policylist_treasurer", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($policies));
    ...
    public function shortlist_post()
    {
        ...
        if (empty($shortlisted)) {
            ...
            if ($res) {
                ...
                storemylog("Shortlist / Unshortlist of policy by User", "api/v1/funeral/shortlist", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], 'Policy shortlisted successfully');
            } else {
                $main_arr['info']['response']['status'] = '0';
                $main_arr['info']['response']['message'] = 'Policy not shortlisted. Try Again.';
                $main_arr['info']['response']['data'] = '';

                storemylog("Shortlist / Unshortlist of policy by User", "api/v1/funeral/shortlist", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Policy not shortlisted. Try Again.');
            }
            ...
            if ($res) {
                ...
                $main_arr['info']['response']['data'] = '';
                storemylog("Shortlist / Unshortlist of policy by User", "api/v1/funeral/shortlist", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], ($shortlisted->status == '1') ? 'Policy unshortlisted successfully' : 'Policy shortlisted successfully');
            } else {
                $main_arr['info']['response']['status'] = '0';
    ...
    public function policydetails_post()
    {
        ...
        $main_arr['info']['response']['message'] = 'Executed Successfully';
        $main_arr['info']['response']['data'] = $data;
        storemylog("Policy Details", "api/v1/funeral/policydetails", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($data));
    ...
    public function policypurchase_post()
    {
        ...
        $main_arr['info']['response']['message'] = 'Executed Successfully';
        $main_arr['info']['response']['data'] = $data;
        storemylog("Policy Purchase Page", "api/v1/funeral/policypurchasepage", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($data));
    ...
    public function policypayment_post()
    {
        ...
        if (strtolower($payment_mode) == 'cash') {
            ...
            storemylog("Policy Payment", "api/v1/funeral/policypayment", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($transactionArray));
            ...
            if (strtolower($resparray[0]['status']) == 'ok') {
                ...
                storemylog("Policy Payment", "api/v1/funeral/policypayment", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($val));
            } else {
                $main_arr['info']['response']['status'] = '0';
                $main_arr['info']['response']['data'] = '';
                $main_arr['info']['response']['message'] = 'Payment was not successful. please try again.';

                storemylog("Policy Payment", "api/v1/funeral/policypayment", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Payment was not successful. please try again.');
            }
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['data'] = '';
            ...
            storemylog("Policy Payment", "api/v1/funeral/policypayment", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Payment mode not supported');
        }
    ...
    public function activepolicy_post()
    {
        ...
        storemylog("Active Policy Lists", "api/v1/funeral/activepolicy", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($data));
    ...
    public function claimspolicy_list_post()
    {
        ...
        storemylog("Policy list dropdown for Claims", "api/v1/funeral/claimpolicy_list", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($data));
    ...
    public function raiseclaim_post()
    {
        ...
        if ($res) {
            ...
            storemylog("Raise Claims", "api/v1/funeral/raiseclaim", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], 'Claim raised successfully');
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = 'Claim not raised. Try Again.';
            $main_arr['info']['response']['data'] = '';

            storemylog("Raise Claims", "api/v1/funeral/raiseclaim", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Claim not raised. Try Again.');
        }
    ...
    public function claimlist_post()
    {
        ...
        foreach ($claimslist as $clm) {
            array_push(
                ...
                array(
                    ...
                    "dod" => $clm->dod,
                    "files" => array(base_url() . 'uploads/claims/' . $clm->document_1, base_url() . 'uploads/claims/' . $clm->document_2),
        ...
        if (!empty($claims)) {
            ...
            storemylog("Claims List", "api/v1/funeral/claimlist", "POST", json_encode($_POST), "1", $_SERVER['REMOTE_ADDR'], json_encode($claims));
        } else {
            $main_arr['info']['response']['status'] = '0';
            $main_arr['info']['response']['message'] = 'No claims raised.';
            $main_arr['info']['response']['data'] = $claims;

            storemylog("Claims List", "api/v1/funeral/claimlist", "POST", json_encode($_POST), "0", $_SERVER['REMOTE_ADDR'], 'Technical Error. Try Again.');
        }
