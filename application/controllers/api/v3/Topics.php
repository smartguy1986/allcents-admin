...
class Topics extends REST_Controller  {
    ...
    public function bycategoryold_post()
	{
        ...
        $main_arr['info']['description']='All Topics List by category';
        $main_arr['info']['generate_time']=date('d-m-Y H:i:s');
        $main_arr['info']['method']='POST';
        $main_arr['paths']['api/v1/topics/bycategory/(:num)']['POST']['summary']='Get list of all the topics by category';
        $main_arr['paths']['api/v1/topics/bycategory/(:num)']['POST']['description']='All the active topics list belongs to a partcular category';
        $main_arr['paths']['api/v1/topics/bycategory/(:num)']['POST']['parameters']='cat_id';
        if(!empty($data))
        {
            $main_arr['info']['response']['status'] = '1';
            $main_arr['info']['response']['data'] = $data;
            ...
            $log_array = array(
                "api_name" => "All Topics By category",
                "api_method" => "POST",
                "api_data" => json_encode($_POST),
                "api_lang" => "EN",
                "api_status" => 1,
    ...
    public function bycategory_post()
	{
        $lang = $this->input->post('lang');
        $cat_id = $this->input->post('cat_id');
        ...
        $mn = date("m");
        $this->db->select('*');
        $this->db->where(array("cat_id"=> $cat_id, "year"=>$yr, "month"=>$mn));
        $ctr = $this->db->get('snr_category_tracker')->result();
        if(count($ctr)>0)
        {
            $this->db->where(array('cat_id'=>$cat_id, "year"=>$yr, "month"=>$mn));
            $this->db->set('views', 'views+1', FALSE);
            $this->db->update('snr_category_tracker');
        }
        else
        {
            $val = array("cat_id"=>$cat_id, "year"=>$yr, "month"=>$mn,"views"=>1);
            $this->db->insert('snr_category_tracker',$val);
        }        
        /* =================================== */
